...
class MCTS_Agent(object):
    ...
    def eval_step(self, state):
        ...
        action_model_probs = self.model_action.predict_proba(np.reshape(np.nan_to_num(np.array([self.dealer, self.hand_strength,
                                                                                              ...
                                                                                              self.prev_winning_prob, self.prev_highest_card])), (1,-1)))

        mcts_probs = sorted(mcts_probs, key=lambda x: x[0])
        self.action_df.loc[len(self.action_df.index)] = {'model_raise': action_model_probs[2],
                                                        'model_check': action_model_probs[1],
                                                        'model_fold': action_model_probs[0],
                                                        'mcts_raise': mcts_probs[2],
                                                        'mcts_check': mcts_probs[1],
                                                        'mcts_fold': mcts_probs[0]}
    ...
    def update(self, state):
        ...
        # hand
        full_hand = rlcardtoeval7(curr_hand) + rlcardtoeval7(curr_public_cards)
        ...
        self.winning_prob = calc_win_prob(rlcardtoeval7(curr_public_cards), rlcardtoeval7(curr_hand), self.num_outs)
